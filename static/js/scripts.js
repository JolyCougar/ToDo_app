...
confirmAddTask.addEventListener('click', function () {
    ...
    addDeleteTaskHandler(newTaskItem);
});
            ...
            .then(data => {
                if (data.success) {
                    ...
                    `;
                        ...
                        addDeleteTaskHandler(completedItem); // Добавляем обработчик для удаления выполненной задачи
                    } else {
                        ...
                        newTaskItem.innerHTML = `
                        <input type="checkbox" id="task${Date.now()}">
                        <label for="task${Date.now()}">${taskItem.querySelector('label').textContent}</label>
                        <button class="delete-task">🗑️</button>
                    `;
                        taskList.appendChild(newTaskItem);
                        taskItem.remove(); // Удаляем задачу из списка выполненных
                        addDeleteTaskHandler(newTaskItem); // Добавляем обработчик для новой задачи
                    }
...
function addDeleteTaskHandler(taskItem) {
